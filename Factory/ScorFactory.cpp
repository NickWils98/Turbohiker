...
ScorFactory::ScorFactory(sf::RenderWindow& w, const sf::Color col,sf::View& v, std::string s, std::shared_ptr<sf::Font>& f, bool u)
...
std::shared_ptr<Entity> ScorFactory::createProp(std::tuple<double, double> size, std::tuple<double, double> position) {
    ...
    std::shared_ptr<Entity> player = std::make_shared<TextFieldSFML>(TextFieldSFML(window, color, sfmlsize, sfmlposition, text, font, toupdate));
