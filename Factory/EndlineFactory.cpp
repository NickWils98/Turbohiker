...
#include "EndlineFactory.h"

EndlineFactory::EndlineFactory(sf::RenderWindow& w, const sf::Color col,sf::View& v)
        : view(v), window(w), color(col){

}

std::shared_ptr<Entity>  EndlineFactory::createProp(std::tuple<double, double> size, std::tuple<double, double> position) {
